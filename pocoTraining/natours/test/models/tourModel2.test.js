...
describe('TDD Tour Model', () => {
  beforeAll(() => {
    Tour.find = jest.fn().mockResolvedValue(expected);
    Tour.create = jest.fn().mockResolvedValue(createdTour);
    Tour.findById = jest.fn().mockResolvedValue(expected[0]);
    const updatedTour = expected[0];
    updatedTour.ratingsAverage = 3;
    updatedTour.ratingsQuantity = 10;
    Tour.findByIdAndUpdate = jest.fn().mockResolvedValue(updatedTour);

    Tour.findByIdAndUpdate = jest.fn().mockResolvedValue(expected[0]);
    Tour.findByIdAndDelete = jest.fn().mockResolvedValue(expected[1]);
  });

  it('Should return entries', async () => {
    const tours = await Tour.find();
    expect(tours).toEqual(expected);
  });

  it('Should create one tour', async () => {
    const tour = await Tour.create(createdTour);
    expect(tour._id).toEqual('5c88fa8cf4afda39709c295a');
  });

  it('Should return one tour', async () => {
    const tour = await Tour.findById('5c88fa8cf4afda39709c2955');
    expect(tour._id).toEqual('5c88fa8cf4afda39709c2955');
  });

  it('Should update one tour', async () => {
    ...
    expect(tour._id).toEqual('5c88fa8cf4afda39709c2955');
    expect(tour.ratingsAverage).toEqual(3);
    expect(tour.ratingsQuantity).toEqual(10);
  });

  it('Should delete one tour', async () => {
    const tour = await Tour.findByIdAndDelete('5c88fa8cf4afda39709c2951');
    expect(tour._id).toBe('5c88fa8cf4afda39709c2951');
  });
});
